<template>
  <div class="wrapper">
    <div class="container">
      ...
      <!-- 添加或修改参数配置对话框 -->
      <el-dialog title="重量阶梯系数" :visible.sync="open" width="600px" append-to-body>
        <el-form ref="form" :model="form" :rules="rules" label-width="120px" class="from-pop">
          <el-form-item label="重量范围" prop="weightRange">
            <el-input class="numrule" v-model="form.weightRange" placeholder="请输入重量范围" />
          </el-form-item>
...
export default {
  ...
  data() {
    return {
      ...
      // 表单校验
      rules: {
        weightRange: [{ required: true, message: '重量范围不能为空', trigger: 'blur' }],
        weightFactor: [{ required: true, message: '重量系数不能为空', trigger: 'blur' }],
      },
    }
  },
  ...
  methods: {
    ...
    getList() {
      ...
      getWeightStepList(data).then((res) => {
    ...
    statusRow(row, index) {
      ...
      let { id, status, weightRange, weightFactor } = row
      status = status == 0 ? 1 : 0
      let data = {
        id: id, // 序号
        status: status, // 状态
